...
const galleryModalImage = (src, alt) => {
    return `<img class="gallery-image" src="${src}" alt="${alt}"/>`;
}
...
document.addEventListener('DOMContentLoaded', (event) => {
    ...
    galleryE.innerHTML = images.reduce((html, element) => {
        const {preview: previewSrc, original: originalSrc, description: alt} = element;
        html += galleryItemHtml(previewSrc, originalSrc, alt);
        return html;
    }, '');
    ...
    galleryE.addEventListener('click', (event) => {
        ...
        lightboxInstance.element().querySelector('.modal-gallery-wrap').innerHTML = galleryModalImage(imageSrc, imageAlt);
